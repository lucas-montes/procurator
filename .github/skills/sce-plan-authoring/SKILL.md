...
- Prompt the user to start a new session with the **shared-context-code** agent to implement `T01`.
- Provide one canonical next command: `/next-task {plan_name} T01`.

## Related skills
- [sce-bootstrap-context](../sce-bootstrap-context/SKILL.md) — required baseline before planning if `context/` is missing.
