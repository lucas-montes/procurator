...
Load and follow [`sce-plan-review`](../skills/sce-plan-review/SKILL.md), then
[`sce-task-execution`](../skills/sce-task-execution/SKILL.md), then
[`sce-context-sync`](../skills/sce-context-sync/SKILL.md).
...
Input:
${input:args:Plan name or path, optionally followed by the task ID (for example `auth-refresh T02`)}

## Behavior
- Keep this prompt as thin orchestration; skill-owned review, implementation, validation, and context-sync details stay in the referenced skills.
...
- If this was the final plan task, run [`sce-validation`](../skills/sce-validation/SKILL.md); otherwise stop after prompting a new session with `/next-task {plan_name} T0X`.
