...
use crate::{
    ...
    application::gerrit::{
        ChangeCommandPort, ChangeQueryPort, CreateChange, CreateChangeInput, PolicyPort,
        UploadPatchSet, UploadPatchSetInput, VoteOnChange, VoteOnChangeInput,
    },
...
async fn resolve_repo_for_user_project(
    ...
    let user = state.db.get_user_by_username(username).await.map_err(|_| {
        (
...
async fn create_change(
    ...
    let repo =
        match resolve_repo_for_user_project(&state, &username, &project_name, &repo_name).await {
            Ok(repo) => repo,
...
async fn upload_patch_set(
    ...
    match use_case.execute(input).await {
        Ok(()) => (
            StatusCode::CREATED,
            Json(serde_json::json!({ "ok": true, "repository_id": repo.id })),
        )
            .into_response(),
        Err(error) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn vote_change(
    ...
    match use_case.execute(input).await {
        ...
        Err(error) => {
            (StatusCode::BAD_REQUEST, format!("Vote rejected: {}", error)).into_response()
        }
    }
}
...
async fn submit_change(
    ...
    match review_repo.update_change_status(change_id, "Merged").await {
        Ok(()) => (
            StatusCode::OK,
            Json(serde_json::json!({ "ok": true, "status": "Merged" })),
        )
            .into_response(),
        Err(error) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn abandon_change(
    ...
    let review_repo = SqliteReviewRepository::new(state.db.clone());
    match review_repo
        .update_change_status(change_id, "Abandoned")
        .await
    {
        Ok(()) => (
            StatusCode::OK,
            Json(serde_json::json!({ "ok": true, "status": "Abandoned" })),
        )
            .into_response(),
        Err(error) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn restore_change(
    ...
    match review_repo.update_change_status(change_id, "New").await {
        Ok(()) => (
            StatusCode::OK,
            Json(serde_json::json!({ "ok": true, "status": "New" })),
        )
            .into_response(),
        Err(error) => (
            StatusCode::INTERNAL_SERVER_ERROR,
